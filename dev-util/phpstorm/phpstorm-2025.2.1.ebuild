...
DESCRIPTION="The PHP IDE that makes you productive"
HOMEPAGE="https://www.jetbrains.com/phpstorm/"
SRC_URI="https://download-cdn.jetbrains.com/webide/PhpStorm-${PV}.tar.gz     -> ${P}.tar.gz"
BUILD_VERSION="252.25557.128"
...
S="${WORKDIR}/PhpStorm-${BUILD_VERSION}"
...
src_prepare() {
	...
	#~ for FD in "${FILES_DELETE[@]}"; do
		#~ rm -rv "${FD}" || die "Failed to remove '${FD}'"
	#~ done
...
src_install() {
	...
	insinto "${dir}"
	doins -r *
	fperms 755 "${dir}"/bin/{phpstorm,phpstorm.sh,fsnotifier,inspect.sh,ltedit.sh,restarter}
	#~ fperms 755 "${dir}"/lib/ReSharperHost/linux-x64/{Rider.Backend,JetBrains.Debugger.Worker,JetBrains.ProcessEnumerator.Worker}
